                    ...
                    if (file_exists($ruta_video) && strlen(trim($archivo_video['basename'])) > 0) {//validamos que exista el archivo
                        ...
                        $return = array(
                            ...
                            'message' => '<a href="' . $this->config->item('motor').'/embed/' . $objBeanVideoSaved->id . '">El archivo subido estará ubicado aquí</a>
                                          <a id="use-this-video" class="btn btn-default col-lg-2 pull-right" href="' . $this->config->item('motor').'/embed/' . $objBeanVideoSaved->id . '#' . $post['descripcion'] . '">Usar este video</a><br>
...
//        }
    ...
    public function getCanalByKey($key_canal)
    {
