...
//    $container = md_elemento_container($ext);
    ...
    public function actualizar_seccion() {
        if ($this->input->is_ajax_request()) {
            $user_id = (int) $this->session->userdata('user_id');            
            $ndetalles =  $this->detalle_secciones_m->obtener_id_detalle_seccion_activos($this->input->post('seccion_id'));            
            $this->estado = ((count($ndetalles)>0)?1:0);                       
            $this->secciones_m->update($this->input->post('seccion_id'), array('estado_migracion' => $this->config->item('migracion:actualizado'), 'nombre' => $this->input->post('nombre'), 'descripcion' => $this->input->post('descripcion'), 'templates_id' => $this->input->post('template'), 'fecha_actualizacion' => date("Y-m-d H:i:s"), 'usuario_actualizacion' => $user_id,'estado' => $this->estado));
            echo json_encode(array("value" => "1"));
        }
    }
    ...
    public function agregar_portada($canal_id, $origen_id = null) {
        if ($this->input->is_ajax_request()) {
            $user_id = (int) $this->session->userdata('user_id');
            $returnValue = 0;
            $portada_id = 0;
            ...
            if (count($resultado) > 0) {
                ...
                $objBeanPortada->origen_id = (empty($origen_id)) ? $canal_id : $origen_id; //$canal_id
                $objBeanPortada->estado = 0;
                $objBeanPortada->fecha_registro = date("Y-m-d H:i:s");
                $objBeanPortada->usuario_registro = $user_id;
                $objBeanPortada->fecha_actualizacion = date("Y-m-d H:i:s");
            ...
            if (!empty($origen_id)) {
                ...
                foreach ($arraySecciones as $puntero => $objTipoSeccion) {
                    if ($objTipoSeccion->id < intval($this->config->item('seccion:perzonalizado'))) {//no se creara secciones personalizadas
                       ...
                       switch ($objTipoSeccion->id) {
                            case $this->config->item('seccion:visto'):
                            case $this->config->item('seccion:comentado'):
                            case $this->config->item('seccion:valorado'):
                            case $this->config->item('seccion:reciente'):
                            $objBeanSeccion->estado = $this->config->item('estado:publicado');
                            break;
                            default:
                        ...
                        $objBeanSeccion->fecha_migracion_actualizacion = '0000-00-00 00:00:00';
                        $objBeanSeccion->grupo_maestros_id = NULL;
                        $this->secciones_m->save($objBeanSeccion);
                    }
                }
            }
...
//            echo json_encode(array("value" => "1"));
    ...
    private function tiene_destacado_publicado($canal_id, $tipo) {
        ...
        if ($tipo == 'canal') {
            ...
            $objPortada = $this->portada_m->get_by(array("origen_id" => $canal_id, "tipo_portadas_id" => $this->config->item('portada:categoria')));
            if (count($objPortada) > 0) {
                $objColeccionSecciones = $this->secciones_m->get_by(array("portadas_id" => $objPortada->id, "tipo_secciones_id" => $this->config->item('seccion:destacado'), "estado" => $this->config->item('estado:publicado')));
                if (count($objColeccionSecciones) > 0) {
                    $returnValue = TRUE;
                }
            }
    ...
    public function eliminar_portada($portada_id) {
        if ($this->input->is_ajax_request()) {
            ...
            $this->procesos_lib->curlGenerarPortadasMiCanalXId($portada_id);
            echo json_encode(array("value" => "1"));
        }
    }
    ...
    public function publicar_portada($portada_id) {
        if ($this->input->is_ajax_request()) {
            ...
            if (count($objPortada) > 0) {
                ...
                elseif($objPortada->tipo_portadas_id == $this->config->item('portada:categoria')){  
                    //verificamos q al menos un maestro esté publicado para activarlo
                    $lista_maestros_publicados = $this->vw_maestro_video_m->get_many_by(array("v" => "v", "categorias_id" => $objPortada->origen_id, "estado" => $this->config->item('video:publicado')));                    
                    if (count($lista_maestros_publicados) > 0) {
                        //validamos que tenga la sección destacado publicada
                        ...
                        if (count($maestroEnSeccion) > 0) {
                            ...
                            if (FALSE) {
                                ...
                                if ($tipo_maestro_registrado > 0) {
                                    ...
                                    $objMaestroCandidato = $this->grupo_maestro_m->get($maestro_id);
                        ...
                        if (FALSE) {
                            ...
                            if ($tipo_maestro_registrado > 0) {
                                ...
                                if ($objMaestroCandidato->tipo_grupo_maestro_id == $tipo_maestro_registrado) {
                                    ...
                                    $returnValue = 2; //el tipo de maestro no es posible agregar
                                }
                            } else {//registramos el primer item
                                $objDetalleSeccion = $this->detalle_secciones_m->get_by(array("grupo_maestros_id" => $maestro_id, "secciones_id" => $seccion_id));
                                $peso = $this->obtenerPeso($seccion_id);
                                $this->detalle_secciones_m->update($objDetalleSeccion->id, array("imagenes_id" => $objImagen->id, "peso" => $peso, "estado" => $this->config->item('estado:publicado'), "estado_migracion" => $this->config->item('migracion:actualizado')));
                                $returnValue = 0;
    ...
    /**
     * Método para obtener el tipo de maestro que está registrado en el detalle de una sección
     * @author Johnny Huamani <johnny1402@gmail.com>
     * @param int $seccion_id
     * @return int
     */
    private function obtener_tipo_maestro_registrado($seccion_id) {
        $returnValue = 0;
        $lista_detalle_seccion = $this->detalle_secciones_m->get_many_by(array("secciones_id" => $seccion_id, "estado" => $this->config->item('estado:publicado')));
        if (count($lista_detalle_seccion) > 0) {
            $count_video = 0;
            $count_lista = 0;
            foreach ($lista_detalle_seccion as $puntero => $objDetalleSeccion) {
                if ($objDetalleSeccion->grupo_maestros_id != NULL) {
                    $objMaestro = $this->grupo_maestro_m->get($objDetalleSeccion->grupo_maestros_id);
                    if (count($objMaestro) > 0) {
                        if ($objMaestro->tipo_grupo_maestro_id == $this->config->item('videos:lista')) {
                            ...
                            if ($objMaestro->tipo_grupo_maestro_id == $this->config->item('videos:video')) {
                                $count_video++;
                            }
                        }
                    }
            ...
            if ($count_lista > $count_video) {
                $returnValue = $this->config->item('videos:lista');
            } else {
                if ($count_lista < $count_video) {
                    $returnValue = $this->config->item('videos:video');
                } else {
                    $returnValue = $this->config->item('videos:video');
                }
            }
    ...
    /**
     * Método para verificar si es una sección excluyente
     * @author Johnny Huamani <johnny1402@gmail.com>
     * @param int $seccion_id
     * @return boolean
     */
    private function es_seccion_excluyente($seccion_id) {
        $returnValue = FALSE;
        $objSeccion = $this->secciones_m->get($seccion_id);
        if (count($objSeccion) > 0) {
            if ($objSeccion->tipo_secciones_id == $this->config->item('seccion:coleccion')) {
                //listamos el detalle de la seccion, para saber que items tiene
                //$lista_detalle_seccion = $this->detalle_secciones_m->get_many_by(array("secciones_id"=>$seccion_id, "estado"=>$this->config->item('estado:publicado')));
                $returnValue = TRUE;
            }
        }
