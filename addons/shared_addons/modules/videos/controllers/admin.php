...
//    $container = md_elemento_container($ext);
    ...
    public function grupo_maestro($canal_id = 0, $maestro_id) {
    ...
    public function itemsMaestros($maestro_id) {
        ...
        if ($maestro_id > 0) {
            ...
            $lista_detalle = $this->grupo_detalle_m->get_many_by(array("grupo_maestro_padre" => $maestro_id));
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post('maestro_id') > 0) {//editare un  maestro
                ...
                if ($this->existeNombreMaestro($this->input->post('titulo'), $this->input->post('canal_id'), $this->input->post('maestro_id'))) {
                    ...
                    $direccion_imagen = UPLOAD_IMAGENES_VIDEOS . '../temp/' . $this->input->post('imagen_maestro');
                    if (file_exists($direccion_imagen)) {
                        ...
                        if (count($arrayTipoImagen) > 0) {
                            ...
                            $this->registrar_imagenes_maestro($objBeanMaestroSaved->id, $arrayImagenes, '../temp/' . $this->input->post('imagen_maestro'));
                            $this->guardarTagsMaestro($objBeanMaestroSaved, $this->input->post());
                        }
                        $returnValue = 0;
                    } else {
    ...
    private function lista_programa($canal_id, $programa_id) {
        $returnValue = array();
        $detalles = $this->grupo_detalle_m->get_many_by(array("grupo_maestro_padre" => $programa_id));
        if (count($detalles) > 0) {
            foreach ($detalles as $puntero => $objDetalle) {
                $objMaestro = $this->grupo_maestro_m->get_by(array("canales_id" => $canal_id, "tipo_grupo_maestro_id" => $this->config->item('videos:lista')));
                if (count($objMaestro) > 0) {
                    array_push($returnValue, $objMaestro);
                }
            }
