...
class Mongo_db 
{
    ...
    // where + order
    public function where_order($collection, $wheres) 
    {
        $result = $this->db->{$collection}->find($wheres)->sort(array("peso" => 1));
     ...
     */
    public function get($collection = "", $limit = NULL, $offset = 0, $order = array()) 
    {
        if (empty($collection)) {
        ...
        $documents = $this->db->{$collection}->find($this->wheres, $this->selects)->limit((int) $limit)->skip((int) $offset)->sort($order);
     ...
     * @return array
     */
    public function get_where_limit($collection = "", $limit = NULL, $offset = NULL, $where = array(), $order = array()) 
    {
        $result = $this->db->{$collection}->find($where)->sort($order)->limit((int) $limit)->skip((int) $offset);
        return iterator_to_array($result);
    }

    /**
     * Where con like y limit
     * @param string $collection
     * @param string $like     
     * @return array
     */
    public function get_where_like($collection = "", $field = "", $like = NULL) 
    {
        $result = $this->db->{$collection}->find(array($field => new MongoRegex($like)));
     ...
     * @return array
     */
    public function all_pagination($collection, $limit, $offset) 
    {
        if ($offset) {
            $videos = $this->db->{$collection}->find()->limit($limit)->skip($offset)->sort(array('_id' => -1));
        } else {
            $videos = $this->db->{$collection}->find()->limit($limit)->sort(array('_id' => -1));
        }
     ...
     * @return int
     */
    public function get_count_where($collection = "", $where = array()) 
    {
        return $this->db->{$collection}->find($where)->count();
    }
    ...
    /**
     * Retorna la consulta según los campos seleccionados
     * @param string $collection
     * @param array $fields
     * @param array $where
     */
    public function get_fields($collection, $fields, $where)
    {
        $result = $this->db->{$collection}->find($where, $fields);
        return iterator_to_array($result);
    }
