...
$(document).on('ready', function() {
    ...
    $(document).on('click', '#submit_cut', function(e) {
        ...
        if ($("#dur_corte").val() == '') {
            $type = 'danger';
            $title = 'Debe realizar un corte';
            $valid = false;
        } else if ($("#dur_corte").val() >= Math.round($("#dur_total").val() * 10) / 10) {
            $type = 'danger';
            $title = 'La duracion del nuevo video debe ser diferente al original';
            $valid = false;
            ...
            $type = 'danger';
            $title = 'Ingrese la descripción del video.';
            $valid = false;
        } else if (values['tematicas'].length == 0) {
            $type = 'danger';
            $title = 'Ingrese las temáticas.';
            $valid = false;
        } else if (values['personajes'].length == 0) {
            $type = 'danger';
            $title = 'Ingrese personajes.';
            $valid = false;
        }
