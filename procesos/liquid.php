...
//set_time_limit(300); // 120 segundos
...
class Liquid extends Exception {
...
//                return $ret;
    ...
    function obtenerDatosMedia($datos) {
        ...
        $url = APIURL . "/medias/" . $datos["codigo"] . "?key=" . $datos["apikey"] . "&filter=id;thumbs;files";
        echo $url . "<br>";
...
//
//
//        $url = APIURL . "/medias/" . $mediaId . "?key=" . $this->apiKey . "&filter=id;thumbs";
//        $response = $this->getXml($url);
//        $mediaxml = new SimpleXMLElement($response);
//        //$jsonmedia=json_encode($media); 
//        $mediaarr = json_decode(json_encode($mediaxml), true);
...
/*
  $liquid = new Liquid();
  $liquid -> obtenernumberOfViews("3b540fec2d40b445f91432821079128d");
 */
?>
